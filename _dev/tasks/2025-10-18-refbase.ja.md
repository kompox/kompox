...
- [x] テスト: 許可/不許可の境界テストを追加[tests]
  - [x] 既存テスト全更新: model.App に RefBase = "file://" + dir + "/" 設定
  - [x] Kompox アプリ直下(file://)での成功ケース
  - [x] 外部KOM(空)での拒否ケース
  - [x] tests/aks-e2e-kom: RefBase ポリシーと KOM 読み込み経路の包括的テスト(13 test cases)
- [x] ドキュメント: ADR 参照追記、索引再生成(`make gen-index`)[adr.readme]
...
- 2025-10-19: CLI の HasLocalFSReference 呼び出しを削除:
  ...
  - 全テスト成功
- 2025-10-19: kompoxopscfg の RefBase 設定を修正:
  - converter.go: configFilePath を絶対パスに変換してから RefBase を設定(相対パスでの不正な RefBase 生成を防止)
  - E2E テストでの file:compose.yml 解決エラーを修正
  - 全テスト成功
- 2025-10-19: v1alpha1 の RefBase 設定を修正:
  - sink_tomodels.go: sourceFile と kompoxAppFilePath を絶対パスに正規化して比較(パス比較の正確性を向上)
  - KOM E2E テストでの RefBase 空文字列エラーを修正
  - 全テスト成功
- 2025-10-19: E2E テスト設定ファイルの KOM 形式への移行:
  - tests/aks-e2e-{easyauth,gitea,gitlab,redmine,volume}: kompoxops.yml.in を kompoxapp.yml.in (KOM 形式) に変換
  - 各テストディレクトリの Makefile, .gitignore, test-run.sh を kompoxapp.yml 参照に更新
  - tests/aks-e2e-{basic,crd,easyauth,gitea,gitlab,redmine}: test-teardown.sh で dns destroy 失敗を無視(`|| true` 追加)
- 2025-10-19: v1alpha1.AppVolumeSpec.Size の柔軟化:
  - types.go: Size フィールドを `int64` から `any` に変更(文字列と数値の両方を受け付ける)
  - sink_tomodels.go: parseVolumeSize ヘルパー関数を追加(int64/int/float64/string "10Gi" 形式をサポート)
  - sink_tomodels_test.go: TestParseVolumeSize を追加(8 test cases)
  - kompoxopscfg との互換性確保(ユーザーは "10Gi" と書ける)
  - 全テスト成功
- 2025-10-19: tests/aks-e2e-kom の作成(tests/aks-e2e-crd から名前変更):
  - RefBase ポリシーの包括的テスト追加(13 test cases)
  - KOM 読み込み経路の検証(Defaults.spec.komPath, --kom-path, KOMPOX_KOM_PATH, --kom-app, KOMPOX_KOM_APP)
  - Defaults.spec.appId の動作検証(有効/無効なAppID)
  - テストフレームワーク改善:
    - describe/run ヘルパー関数(BDD スタイル)
    - カラー出力(TEST=青、PASS=緑、FAIL=赤、すべて太字反転)
    - Makefile: kompoxapp-*.yml.in からの自動生成ルール
  - 全テスト成功
