...
func newCmdConfig() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "version=%s workspace=%s provider=%s(%s) cluster=%s domain=%s app=%s\n",
				cfg.Version, cfg.Workspace.Name, cfg.Provider.Name, cfg.Provider.Driver, cfg.Cluster.Name, domain, cfg.App.Name)
			return nil
		},
	}
