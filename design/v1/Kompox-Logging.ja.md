...
`CMD` ログは「コマンド実行フェーズに入った」ことを表す。したがって、root の `PersistentPreRunE` が成功し、実行用の context と logger が初期化された後にのみ開始ログを出力する。

終了ログは `main()` で最終 `exitCode` を確定した後に出力する。

フラグ構文エラー、未知コマンド、ヘルプ表示など、コマンド実行前に処理が終了したケースでは `CMD` 開始・終了ログを出力しない。これらのケースでは human-readable な CLI メッセージのみを stderr に出力する。

`CMD` 終了ログを出力するかどうかの判定は、エラーメッセージ文字列や Cobra の具体的なエラー型ではなく、「実行開始済みの context が作成されているか」に基づいて行う。
...
`runId` は CLI 起動時に root の `PersistentPreRunE` で一度だけ生成され、ロガーに付与される。以降のすべてのログ出力はこの `runId` を継承する。
...
- `CMD` 終了ログは、実行開始済みの context が存在する場合にのみ出力できる
