...
- [x] `from: - {}` が出る再現ケースをテスト化する。
- [x] same-namespace 許可の表現を zero value 非依存へ変更する。
- [x] 既存の `kube-system` / ingress namespace 許可との整合を確認する。
- [x] 回帰テストを追加し、生成 YAML に `from: - {}` が出ないことを確認する。
