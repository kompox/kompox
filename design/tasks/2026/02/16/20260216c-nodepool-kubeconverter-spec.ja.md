...
- 対象:
  - [Kompox-KubeConverter] の NodeSelector ラベル契約に関する節を更新
  - `deployment.pool` / `deployment.zone` と `kompox.dev/node-pool` / `kompox.dev/node-zone` のマッピング責務を明記
...
- KubeConverter は `deployment.pool/zone` をノード選択ラベルへ写像する契約を維持する。
...
- [ ] [Kompox-KubeConverter] の既存 NodeSelector/ラベル仕様記述を確認する。
- [ ] `deployment.pool/zone` と NodePool 抽象の関係を仕様として追記する。
- [ ] `kompox.dev/node-pool` / `kompox.dev/node-zone` ラベル契約の維持を明記する。
- [ ] zone 正規化責務を provider driver 側に置く方針を明記する。
- [ ] [Kompox-ProviderDriver] / [Kompox-ProviderDriver-AKS] / [K4x-ADR-019] との整合を確認する。
...
- [Kompox-KubeConverter] に `deployment.pool/zone` と NodePool 抽象の責務分離が記載されている。
