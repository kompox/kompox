...
// Converter is a provider-agnostic plan of Kubernetes objects derived from a Kompox App.
//
// Design:
// - NewConverter constructs the provider-agnostic "plan" from an App (Parse/Plan phase).
// - Bind* methods resolve storage strategy (provider-dependent) and return a VolumeBinding (Bind phase).
// - Build assembles final Kubernetes objects using the plan + binding (Assemble phase).
//
// This keeps Compose translation independent from provider details while enabling pluggable binding.
...
// VolumeBinding represents storage binding results.
// ClaimNameByVolume maps logical app volume name -> PVC claim name used in Pod volumes.
// Objects includes PV/PVC (static) or PVC (dynamic) manifests generated by the binding strategy.
...
// BindStaticPVs binds logical volumes to static PV/PVCs using a provider driver and assigned instances.
// It injects CSI details via the driver and returns the PVC claim names to be used by Pods.
func (c *Converter) BindStaticPVs(ctx context.Context, drv providerdrv.Driver, volumeInstances map[string]VolumeInstanceInfo) (VolumeBinding, error) {
	if c.Project == nil || c.NSName == "" {
		return VolumeBinding{}, fmt.Errorf("convert must be called before binding")
	}
	if drv == nil {
		return VolumeBinding{}, fmt.Errorf("provider driver is required for static PV binding")
	}
	...
	for _, av := range c.App.Volumes {
		...
		if !ok || inst.Handle == "" {
			return VolumeBinding{}, fmt.Errorf("volume %s has no assigned handle; provide volumeInstances[%s].Handle", av.Name, av.Name)
		}
		volHandle := inst.Handle
		sizeBytes := av.Size
		volHASH := naming.VolumeHash(volHandle)
		pvName := fmt.Sprintf("kompox-%s-%s-%s", av.Name, c.HashID, volHASH)
		sizeQty := bytesToQuantity(sizeBytes)

		vc, err := drv.VolumeClass(ctx, c.Cls, c.App, av)
		if err != nil {
			return VolumeBinding{}, fmt.Errorf("volume class resolve failed: %s: %w", av.Name, err)
		}
		...
		if csiDriver == "" {
			return VolumeBinding{}, fmt.Errorf("provider driver did not supply CSIDriver for volume %s", av.Name)
		}
		...
		pv := &corev1.PersistentVolume{ObjectMeta: metav1.ObjectMeta{Name: pvName, Labels: c.CommonLabels}, Spec: pvSpec}
		...
		pvc := &corev1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Name: pvName, Namespace: c.NSName, Labels: c.CommonLabels}, Spec: pvcSpec}

		objs = append(objs, pv, pvc)
		claimNames[av.Name] = pvName
	}

	return VolumeBinding{ClaimNameByVolume: claimNames, Objects: objs}, nil
}

// BindDynamicPVCs builds PVCs only (dynamic provisioning) and returns their claim names.
// Use this when relying on a StorageClass instead of pre-provisioned volumes.
func (c *Converter) BindDynamicPVCs(storageClass string) (VolumeBinding, error) {
	if c.Project == nil || c.NSName == "" {
		return VolumeBinding{}, fmt.Errorf("convert must be called before binding")
	}
	...
	for _, av := range c.App.Volumes {
		pvcName := fmt.Sprintf("kompox-%s-%s", av.Name, c.HashID)
		qty := bytesToQuantity(av.Size)
		spec := corev1.PersistentVolumeClaimSpec{
			AccessModes: []corev1.PersistentVolumeAccessMode{corev1.ReadWriteOnce},
			Resources:   corev1.VolumeResourceRequirements{Requests: corev1.ResourceList{corev1.ResourceStorage: qty}},
		}
		if storageClass != "" {
			spec.StorageClassName = strPtr(storageClass)
		}
		pvc := &corev1.PersistentVolumeClaim{
			ObjectMeta: metav1.ObjectMeta{Name: pvcName, Namespace: c.NSName, Labels: c.CommonLabels},
			Spec:       spec,
		}
		objs = append(objs, pvc)
		claim[av.Name] = pvcName
	}
	return VolumeBinding{ClaimNameByVolume: claim, Objects: objs}, nil
}
...
func (c *Converter) Build(binding VolumeBinding) ([]runtime.Object, []string, error) {
	if c.Project == nil || c.NSName == "" {
	...
	for _, av := range c.App.Volumes {
		claimName, ok := binding.ClaimNameByVolume[av.Name]
		if !ok || claimName == "" {
	...
	objs = append(objs, binding.Objects...)
